...
//  }
  ...
  for( ; ; ) {
    ...
    if( VERBOSE ) {
      ...
      p += sprintf(p, "[vegas] "
        "%3d %9d pts",// COUNT ":  " NUMBER " pts",
        state.niter + 1, t->neval);

      for( comp = 0; comp < t->ncomp; ++comp ) {
        cCumulants *c = &state.cumul[comp];
          real rel_err;
          if (fabs(c->avg)>1e-16) rel_err = fabs(c->err/c->avg);
          else rel_err = 0.0; 
          
           
          
         p += sprintf(p," : %1.5le +- %1.3le (%1.2le) chisq %2.1le / %3d", c->avg, c->err,
             rel_err,
              c->chisq, state.niter);
//         p += sprintf(p, " : "
//          REAL " +- " REAL "  \tchisq " REAL " (" COUNT " df)",
//          //comp + 1,
//           c->avg, c->err, c->chisq, state.niter);
      }
