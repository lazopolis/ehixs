...
class Bin {
  ...
  Bin()
    : running_f(0.0), running_f2(0.0), avg_f(0.0), avg_err(0.0), intra_point_f(0.0), running_chi_sq(0.0), iteration_number(0)
  {}
  ...
  /** \brief Update all internals
    *
    * Computes the new value error and chi-squared of the integral. Parameter is the number of points that have been thrown by VEGAS since tha last call to update(). */
  void update(unsigned);
  /** \brief End of point accumulation
    *
    * This functions ends and restart the accumulation of points belonging to the same physical point. */
  void end();
  /** \brief Return value */
